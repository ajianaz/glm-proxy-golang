...
func makeTestKeyStore(t *testing.T) *storage.KeyStore {
	return makeTestKeyStoreWithKey(t, "pk_test")
}

func makeTestKeyStoreWithKey(t *testing.T, keyName string) *storage.KeyStore {
	t.Helper()
	dir := t.TempDir()
...
func newTestOpenAIProxy(t *testing.T, upstreamURL string) (*OpenAIProxy, *storage.KeyStore) {
	t.Helper()
...
func TestOpenAIProxy_NonStreaming(t *testing.T) {
	...
	p, _ := newTestOpenAIProxy(t, upstream.URL)
...
func TestOpenAIProxy_NonStreaming_CostHeaderParsed(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("x-litellm-response-cost", "0.012")
		resp := map[string]interface{}{
			"id":      "chatcmpl-cost",
			"choices": []interface{}{},
			"usage": map[string]interface{}{
	...
	body := strings.NewReader(`{"model":"gpt-4","messages":[{"role":"user","content":"hi"}],"stream":false}`)
	req := httptest.NewRequest("POST", "/v1/chat/completions", body)
	req.Header.Set("Content-Type", "application/json")
	...
	updated, _ := p.Store.FindKey("pk_test")
	if updated.TotalSpendUSD != 0.012 {
		t.Fatalf("expected total spend 0.012, got %f", updated.TotalSpendUSD)
	}
}
...
func TestOpenAIProxy_NonStreaming_UpdatesStore(t *testing.T) {
	...
	// Synchronous tracking now — no sleep needed
	updated, _ := p.Store.FindKey("pk_test")
